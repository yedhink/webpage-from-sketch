...
class Sampler:
    ...
    def convert_batch_of_images(self, op_dir, pngs_path, get_corpus_bleu,
                                original_guis_filepath, style):
        ...
        for filename in all_filenames:
            if filename.find('.png') != -1:
                input_img = "{}/{}".format(pngs_path, filename)
                try:
                    self.convert_single_image(
                        ...
                        get_sentence_bleu=0,
                        original_gui_filepath=input_img,
                        style=style)
                    generated_count += 1
                except:
                    print("Error with GUI / HTML generation:",
                          sys.exc_info()[0])
                    print(sys.exc_info())
                    continue
    ...
    def convert_single_image(self, op_dir, input_img, print_generated_output,
        ...
        # Retrieve sample ID
        png_filename = os.path.basename(input_img)
        ...
        generated_gui, gui_output_filepath = self.generate_gui(
            input_img,
    ...
    def generate_gui(self, input_img, print_generated_output, sample_id,
        ...
        gui_output_filepath = self.write_gui_to_disk(generated_gui, sample_id,
                                                     op_dir)
    ...
    def generate_html(self,
        ...
        if compiled_website != 'HTML Parsing Error':
            output_filepath = "{}/{}.html".format(op_dir, sample_id)
    ...
    def write_gui_to_disk(self, gui_array, sample_id, op_dir):
        gui_output_filepath = "{}/{}.gui".format(op_dir, sample_id)
