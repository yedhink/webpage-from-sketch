...
class Dataset:
    ...
    def split_datasets(self, validation_split):
        ...
        training_path, validation_path = self.split_samples(
    ...
    @classmethod
    def create_generator(cls, training_dataset, max_sequences):
        img_features, text_features = Dataset.load_data(training_dataset)
        total_sequences = 0
        ...
        data_gen = Dataset.data_generator(text_features, img_features,
                                          max_sequences, tokenizer, vocab_size)
        return data_gen, steps_per_epoch

    @classmethod
    def data_generator(cls, text_features, img_features, max_sequences,
        ...
        while 1:
            for i in range(0, len(text_features), 1):
                ...
                for j in range(i, min(len(text_features), i + 1)):
                    ...
                    in_img, in_seq, out_word = Dataset.process_data_for_generator(
                        [desc], [image], max_sequences, tokenizer, vocab_size)
    ...
    @classmethod
    def process_data_for_generator(cls, texts, features, max_sequences,
                                   tokenizer, vocab_size):
        ...
        for img_no, seq in enumerate(sequences):
            for i in range(1, len(seq)):
                ...
                out_seq = to_categorical([out_seq], num_utils=vocab_size)[0]
    ...
    @classmethod
    def load_data(cls, training_dataset):
        text = []
        images = []
        all_filenames = os.listdir(training_dataset)
        all_filenames.sort()
        for filename in all_filenames:
            if filename[-3:] == "npz":
                image = np.load(training_dataset + '/' + filename)
                images.append(image['features'])
            elif filename[-3:] == 'gui':
                file = open(training_dataset + '/' + filename, 'r')
                texts = file.read()
                file.close()
    ...
    def create_data_folders(self):
        training_path = "{}/{}".format(
            os.path.dirname(self.data_input_folder), TRAINING_SET_NAME)
        validation_path = "{}/{}".format(
            os.path.dirname(self.data_input_folder), VALIDATION_SET_NAME)
    ...
    def copy_files_to_folders(self, sample_ids, op_dir):
        copied_count = 0
        for sample_id in sample_ids:
            sample_id_input_img = "{}/{}.png".format(self.data_input_folder,
                                                     sample_id)
            sample_id_gui_path = "{}/{}.gui".format(self.data_input_folder,
                                                    sample_id)
            if os.path.exists(sample_id_input_img) and os.path.exists(
                    sample_id_gui_path):
                output_input_img = "{}/{}.png".format(op_dir, sample_id)
                output_gui_path = "{}/{}.gui".format(op_dir, sample_id)
                shutil.copyfile(sample_id_input_img, output_input_img)
                shutil.copyfile(sample_id_gui_path, output_gui_path)
                copied_count += 1
        print("Moved {} files from {} to {}".format(
            copied_count, self.data_input_folder, op_dir))
    ...
    def populate_sample_ids(self):
        ...
        for f in os.listdir(full_path):
            if f.find(".gui") != -1:
                ...
                if os.path.isfile("{}/{}.png".format(self.data_input_folder,
                                                     file_name)):
    ...
    def get_all_id_sets(self, validation_split, sample_ids):
        ...
        print(
            "Splitting datasets, training samples: {}, validation samples: {}".
            format(train_count, val_count))
        train_set, val_set = self.split_paths(sample_ids, train_count,
                                              val_count)
    ...
    def split_paths(self, sample_ids, train_count, val_count):
        ...
        for sample_id in sample_ids:
            f = open(
                "{}/{}.gui".format(self.data_input_folder, sample_id),
                'r',
                encoding='utf-8')
