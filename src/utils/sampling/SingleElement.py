...
class SingleElement:
    def __init__(self, key, parent_SingleElement, content_holder):
        self.key = key
        self.parent = parent_SingleElement
        self.children = []
    ...
    def rendering_function(self, key, value):
        if key.find("btn") != -1:
            value = value.replace(TEXT_PLACE_HOLDER, Common.get_random_text())
        elif key.find("title") != -1:
            value = value.replace(
                TEXT_PLACE_HOLDER,
                Common.get_random_text(length_text=5, space_number=0))
            # value = value.replace(TEXT_PLACE_HOLDER, Common.get_random_text(length_text=5, space_number=0))
        elif key.find("text") != -1:
            value = value.replace(
                TEXT_PLACE_HOLDER,
                Common.get_random_text(
