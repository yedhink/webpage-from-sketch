...
class ImagePreprocessor:
    ...
    def build_image_dataset(self, data_input_folder, augment_data=True):
        ...
        print("Converting images from {} into arrays, augmentation: {}".format(
            data_input_folder, augment_data))
        resized_img_arrays, sample_ids = self.get_resized_images(
            data_input_folder)

        if augment_data == 1:
            self.augment_and_save_images(resized_img_arrays, sample_ids,
                                         data_input_folder)
        else:
            self.save_resized_img_arrays(resized_img_arrays, sample_ids,
                                         data_input_folder)

    def get_img_features(self, input_img):
        img_features = self.resize_img(input_img)
        assert (img_features.shape == (256, 256, 3))
        return img_features

    def save_resized_img_arrays(self, resized_img_arrays, sample_ids, op_dir):
        count = 0
        for img_arr, sample_id in zip(resized_img_arrays, sample_ids):
            npz_filename = "{}/{}.npz".format(op_dir, sample_id)
            ...
            count += 1
        print("Saved down {} resized images to folder {}".format(
            count, op_dir))
        del resized_img_arrays

    def augment_and_save_images(self, resized_img_arrays, sample_ids,
                                data_input_folder):
        datagen = ImageDataGenerator(
            ...
            zoom_range=0.05)
        keras_generator = datagen.flow(
            resized_img_arrays, sample_ids, batch_size=1)
        count = 0
        for i in range(len(resized_img_arrays)):
            ...
            assert np.array_equal(img_arr, retrieve)
            count += 1
        print("Saved down {} augmented images to folder {}".format(
            count, data_input_folder))
        del resized_img_arrays
    ...
    def get_resized_images(self, pngs_input_folder):
        ...
        for png_file_path in png_files:
            input_img = "{}/{}".format(pngs_input_folder, png_file_path)
            sample_id = png_file_path[:png_file_path.find('.png')]
            resized_img_arr = self.resize_img(input_img)
    ...
    def resize_img(self, png_file_path):
        ...
        img_adapted = cv2.adaptiveThreshold(img_grey, 255,
                                            cv2.ADAPTIVE_THRESH_MEAN_C,
                                            cv2.THRESH_BINARY, 101, 9)
        img_stacked = np.repeat(img_adapted[..., None], 3, axis=2)
        resized = cv2.resize(
            img_stacked, (200, 200), interpolation=cv2.INTER_AREA)
