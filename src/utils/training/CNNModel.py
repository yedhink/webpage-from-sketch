...
class CNNModel():
    def __init__(self, m_op_path, m_json=None, m_weight=None):

        # Create model output path
        self.m_op_path = m_op_path

        # If we have an existing model json / weights, load in that model
        if m_json is not None and m_weight is not None:
            self.model = self.load_model(m_json, m_weight)
    ...
    def load_model(self, m_json, m_weight):
        json_file = open(m_json, 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        loaded_model = model_from_json(loaded_model_json)
        loaded_model.load_weights(m_weight)
        return loaded_model

    def save_model(self):
        model_json = self.model.to_json()
        with open("{}/model_json.json".format(self.m_op_path),
                  "w") as json_file:
            json_file.write(model_json)
        self.model.save_weights("{}/weights.h5".format(self.m_op_path))
    ...
    def create_model(self):
        ...
        language_model = Embedding(
            vocab_size, 50, input_length=MAX_LENGTH,
    ...
    def train(self, training_path, validation_path, epochs):
        ...
        training_generator, train_steps_per_epoch = Dataset.create_generator(
            training_path, max_sequences=MAX_SEQ)
        validation_generator, val_steps_per_epoch = Dataset.create_generator(
            validation_path, max_sequences=MAX_SEQ)
    ...
    def construct_callbacks(self, validation_path):
        checkpoint_filepath = "{}/".format(
            self.m_op_path
        ) + "weights-epoch-{epoch:04d}--val_loss-{val_loss:.4f}--loss-{loss:.4f}.h5"
        csv_logger = CSVLogger("{}/training_val_losses.csv".format(
            self.m_op_path))
